...
export const getImages = (type) => {
  const copyArray = [...imageCollection[type]];
const mirrorArray = copyArray.map((item) => {
  return {
  ...item,
  id: `${item.id}-1`}
})
const gameTable = [...copyArray, ...mirrorArray]

return (RANDOMIZED? gameTable.sort(() => 0.5 - Math.random()) : gameTable)
};
